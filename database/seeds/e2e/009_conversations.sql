...
SELECT ('00000000-0000-0000-0000-' || lpad((400+n)::text,12,'0'))::uuid,
       ...
       CASE WHEN n = 20 THEN '00000000-0000-0000-0000-000000000303'::uuid ELSE ('00000000-0000-0000-0000-' || lpad((300 + ((n-1)%29+1))::text,12,'0'))::uuid END,
...
ON CONFLICT ("Id") DO UPDATE SET "Status"=EXCLUDED."Status", "AutomationMode"=EXCLUDED."AutomationMode", "AssignedUserId"=EXCLUDED."AssignedUserId", "UpdatedAt"=EXCLUDED."UpdatedAt";
