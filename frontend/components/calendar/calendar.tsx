...
export function CalendarShell(props: CalendarProps) {
  return <div className="grid gap-4"><CalendarToolbar {...props} /><CalendarFilterBar {...props} />{props.loading ? <CalendarSkeleton view={props.view} /> : props.appointments.length === 0 ? <CalendarEmptyState onCreate={props.onCreate} /> : <CalendarViewContent {...props} />}</div>;
}
...
export function CalendarToolbar({ date, view, onDateChange, onViewChange, onPrevious, onNext, onToday, onRefresh, realtimeStatus = "offline" }: CalendarProps) {
  const title = new Date(`${date}T12:00:00`).toLocaleDateString("pt-BR", { month: "long", year: "numeric" });
  return <Card className="p-3"><div className="flex flex-wrap items-center justify-between gap-3 max-sm:flex-col max-sm:items-stretch"><div className="flex items-center gap-2 max-sm:justify-between"><Button onClick={onToday} size="sm" variant="outline">Hoje</Button><Button aria-label="Período anterior" onClick={onPrevious} size="sm" variant="outline">←</Button><Button aria-label="Próximo período" onClick={onNext} size="sm" variant="outline">→</Button><FormField htmlFor="calendar-date" label="Data"><Input aria-label="Escolher data" className="w-36" id="calendar-date" onChange={event => onDateChange(event.target.value)} type="date" value={date} /></FormField><Button aria-label="Atualizar agenda" onClick={onRefresh} size="sm" variant="outline">Atualizar</Button><span className="hidden text-sm font-semibold capitalize text-slate-800 sm:inline">{title}</span></div><div className="flex items-center justify-end gap-3 max-sm:flex-wrap max-sm:justify-between"><span className="text-xs text-slate-500">Horários em America/Recife</span><RealtimeIndicator status={realtimeStatus} /><CalendarViewSwitcher onChange={onViewChange} value={view} /></div></div></Card>;
}
...
export function CalendarWeekView({ appointments, date, onOpen }: CalendarProps) {
