...
export default function AppointmentsPage() {
  ...
  const confirm = useMutation({ mutationFn: (item: AppointmentDetail) => appointmentsApi.confirm(api, item.appointment.id, item.version, crypto.randomUUID()), onSuccess: async () => { await refresh(); setDetail(null); } });
  const cancel = useMutation({ mutationFn: (item: AppointmentDetail) => appointmentsApi.cancel(api, item.appointment.id, "Cancelamento administrativo", item.version, crypto.randomUUID()), onSuccess: async () => { await refresh(); setDetail(null); } });
