...
export default function AuditPage() {
  const api = useApi(); const { user } = useAuth(); const [action, setAction] = useState(""); const [resourceType, setResourceType] = useState(""); const allowed = can(user, "manageCatalog"); const debouncedAction = useDebouncedValue(action); const debouncedResourceType = useDebouncedValue(resourceType);
  const audit = useQuery({ enabled: allowed, placeholderData: keepPreviousData, queryKey: ["audit", debouncedAction, debouncedResourceType], queryFn: () => auditApi.search(api, { page: 1, pageSize: 100, action: debouncedAction, resourceType: debouncedResourceType }) });
