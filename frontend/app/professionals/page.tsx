...
export default function ProfessionalsPage() {
  ...
  const professionals = useQuery({ queryKey: ["professionals"], queryFn: () => catalogApi.listProfessionals(api) });
  const [units, specialties] = useQueries({ queries: [{ queryKey: ["units"], queryFn: () => catalogApi.listUnits(api) }, { queryKey: ["specialties"], queryFn: () => catalogApi.listSpecialties(api) }] });
  ...
  return <ProtectedShell><PageContainer><PageHeader actions={allowed ? <Button disabled={!formReady} onClick={() => setEditing(null)}><Icon name="professionals" />Novo profissional</Button> : undefined} description="Cadastre profissionais, acompanhe status e consulte a agenda dos próximos dias." pathname="/professionals" title="Profissionais" />
