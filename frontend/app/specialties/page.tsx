...
export default function SpecialtiesPage() {
  ...
  const specialties = useQuery({ queryKey: ["specialties"], queryFn: () => catalogApi.listSpecialties(api) }); const [editing, setEditing] = useState<Specialty | null | undefined>(); const [dependencies, setDependencies] = useState<{ specialty: Specialty; data: SpecialtyDependencies } | null>(null); const allowed = can(user, "manageCatalog");
  ...
  return <ProtectedShell><PageContainer><PageHeader actions={allowed ? <Button onClick={() => setEditing(null)}><Icon name="specialties" />Nova especialidade</Button> : undefined} description="Organize especialidades e verifique dependências antes de alterar o status." pathname="/specialties" title="Especialidades" />
