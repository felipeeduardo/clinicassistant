...
export default function PlatformPage() {
  ...
  const [tenants, users, clinics, dashboard] = useQueries({ queries: [
    ...
    { queryKey: ["platform", "dashboard", "30d"], queryFn: () => platformApi.dashboard(api, "30d"), enabled: user?.role === "PlatformAdmin" },
  ] });
  ...
  const loading = tenants.isLoading || users.isLoading || clinics.isLoading || dashboard.isLoading;
  return <ProtectedShell><PageContainer>
    ...
    {dashboard.data && <Card className="mt-6"><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold text-slate-950">Funil comercial · últimos 30 dias</h2><p className="mt-1 text-sm text-slate-500">Indicadores calculados a partir dos leads persistidos.</p></div><StatusBadge tone="info">{dashboard.data.commercial.conversionRate.toLocaleString("pt-BR")}% ganhos</StatusBadge></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><FunnelMetric label="Novos" value={dashboard.data.commercial.new} /><FunnelMetric label="Demonstrações" value={dashboard.data.commercial.demoCompleted} /><FunnelMetric label="Pilotos" value={dashboard.data.commercial.pilot} /><FunnelMetric label="Clientes" value={dashboard.data.commercial.won} /><FunnelMetric label="Perdidos" value={dashboard.data.commercial.lost} /></div></Card>}
    ...
    <RequestFeedback error={tenants.error || users.error || clinics.error || dashboard.error || status.error} />
...
function FunnelMetric({ label, value }: { label: string; value: number }) { return <div className="rounded-control border border-border-system bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold text-slate-950">{value}</p></div>; }
