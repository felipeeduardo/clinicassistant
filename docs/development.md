...
Para desenvolvimento fora de containers, inicie as dependências com `docker compose up postgres rabbitmq redis -d`, então execute `dotnet restore backend/ClinicAssistant.sln`, `dotnet build backend/ClinicAssistant.sln` e `dotnet test backend/ClinicAssistant.sln` na raiz.
