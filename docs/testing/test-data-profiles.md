...
`minimal` cria uma clínica, 2 usuários, 1 unidade, 2 especialidades, 2 profissionais, 5 pacientes, 10 regras de disponibilidade, 3 consultas, 2 conversas, 10 mensagens e uma integração WhatsApp. A integração é `Fake` por padrão; para o smoke local com Twilio Sandbox, use `WHATSAPP_PROVIDER=Twilio` e forneça `TWILIO_WHATSAPP_FROM` (e opcionalmente `TWILIO_INTEGRATION_KEY`).
