...
public sealed class ClinicCatalogService(ClinicAssistantDbContext db, TenantAccessGuard tenantGuard) : IClinicCatalogService
{
    ...
    public async Task<SpecialtyResponse> CreateSpecialtyAsync(SpecialtyRequest request, CancellationToken ct) { await new SpecialtyRequestValidator().ValidateAndThrowAsync(request, ct); var tenantId = tenantGuard.RequireTenantId(); var entity = new Specialty(tenantId, request.Name, request.Description); db.Specialties.Add(entity); db.AuditRecords.Add(new AuditRecord(tenantId, null, "specialty.created", "Specialty", entity.Id, "Succeeded", "Specialty created by clinic administration.")); await db.SaveChangesAsync(ct); return Map(entity); }
    public async Task<SpecialtyResponse> UpdateSpecialtyAsync(Guid id, SpecialtyRequest request, CancellationToken ct) { await new SpecialtyRequestValidator().ValidateAndThrowAsync(request, ct); var entity = await Require(db.Specialties, id, ct); entity.Update(request.Name, request.Description); db.AuditRecords.Add(new AuditRecord(tenantGuard.RequireTenantId(), null, "specialty.updated", "Specialty", id, "Succeeded", "Specialty updated by clinic administration.")); await db.SaveChangesAsync(ct); return Map(entity); }
    public async Task<SpecialtyDependenciesResponse> GetSpecialtyDependenciesAsync(Guid id, CancellationToken ct) { await Require(db.Specialties, id, ct); var professionals = await db.ProfessionalSpecialties.CountAsync(x => x.SpecialtyId == id, ct); var futureAppointments = await db.Appointments.CountAsync(x => x.SpecialtyId == id && x.StartsAt >= DateTimeOffset.UtcNow && x.Status != ClinicAssistant.Domain.Scheduling.AppointmentStatus.Cancelled, ct); return new(professionals == 0 && futureAppointments == 0, professionals, futureAppointments); }
    public async Task SetSpecialtyStatusAsync(Guid id, string status, CancellationToken ct) { var entity = await Require(db.Specialties, id, ct); if (!Enum.TryParse<CatalogStatus>(status, true, out var value)) throw new InvalidOperationException("Invalid specialty status."); if (value == CatalogStatus.Inactive && !(await GetSpecialtyDependenciesAsync(id, ct)).CanDeactivate) throw new InvalidOperationException("The specialty has active dependencies and cannot be deactivated."); entity.SetStatus(value); db.AuditRecords.Add(new AuditRecord(tenantGuard.RequireTenantId(), null, "specialty.status_changed", "Specialty", id, "Succeeded", $"Specialty status set to {value}.")); await db.SaveChangesAsync(ct); }
    public async Task DeleteSpecialtyAsync(Guid id, CancellationToken ct) { var entity = await Require(db.Specialties, id, ct); if (!(await GetSpecialtyDependenciesAsync(id, ct)).CanDeactivate) throw new InvalidOperationException("The specialty has dependencies and cannot be deleted."); db.Specialties.Remove(entity); db.AuditRecords.Add(new AuditRecord(tenantGuard.RequireTenantId(), null, "specialty.deleted", "Specialty", id, "Succeeded", "Specialty deleted with no dependencies.")); await db.SaveChangesAsync(ct); }
