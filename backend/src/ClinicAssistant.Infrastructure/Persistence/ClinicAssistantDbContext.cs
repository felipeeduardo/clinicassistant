...
public sealed class ClinicAssistantDbContext(DbContextOptions<ClinicAssistantDbContext> options, ITenantContext? tenantContext = null) : DbContext(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<WhatsAppChannel>(entity => { entity.ToTable("whatsapp_channels"); entity.Property(x => x.Provider).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.Status).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.NumberOrigin).HasConversion<string>().HasMaxLength(48); entity.Property(x => x.CurrentUsage).HasConversion<string>().HasMaxLength(64); entity.Property(x => x.OnboardingStatus).HasConversion<string>().HasMaxLength(48); entity.Property(x => x.PhoneNumber).HasMaxLength(32).IsRequired(); entity.Property(x => x.NormalizedPhoneNumber).HasMaxLength(32).IsRequired(); entity.Property(x => x.DisplayPhoneNumber).HasMaxLength(80); entity.Property(x => x.ProviderSenderId).HasMaxLength(160); entity.Property(x => x.ValidationMessage).HasMaxLength(1000); entity.Property(x => x.FailureReason).HasMaxLength(1000); entity.HasIndex(x => x.NormalizedPhoneNumber).IsUnique().HasFilter("\"Status\" = 'Active'"); entity.HasIndex(x => x.TenantId); entity.HasIndex(x => new { x.TenantId, x.IsDefault }).HasFilter("\"Status\" = 'Active'"); entity.HasQueryFilter(x => IsPlatformAdmin || (CurrentTenantId.HasValue && x.TenantId == CurrentTenantId.Value)); });
        ...
        modelBuilder.Entity<OperationalNotification>(entity => { entity.ToTable("operational_notifications"); entity.Property(x => x.Type).HasConversion<string>().HasMaxLength(48); entity.Property(x => x.Severity).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.Status).HasConversion<string>().HasMaxLength(32); entity.Property(x => x.CorrelationId).HasMaxLength(128); entity.HasIndex(x => new { x.TenantId, x.ConversationId, x.Type }).IsUnique(); entity.HasIndex(x => new { x.TenantId, x.Status, x.CreatedAt }); entity.HasQueryFilter(x => IsPlatformAdmin || (CurrentTenantId.HasValue && x.TenantId == CurrentTenantId.Value)); });
