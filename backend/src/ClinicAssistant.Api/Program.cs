...
try
{
    ...
    builder.Services.AddAuthorization(options =>
    {
        ...
        options.AddPolicy("ConversationOperators", policy => policy.RequireRole("ClinicAdmin", "Receptionist"));
    ...
    platform.MapGet("/tenants/{tenantId:guid}/whatsapp/channels", async (Guid tenantId, IPlatformAdministrationService service, CancellationToken ct) => Results.Ok(await service.GetWhatsAppChannelsAsync(tenantId, ct)));
    platform.MapPost("/tenants/{tenantId:guid}/whatsapp/channels", async (Guid tenantId, CreateWhatsAppChannelRequest request, IPlatformAdministrationService service, CancellationToken ct) => Results.Created($"/api/platform/tenants/{tenantId}/whatsapp/channels", await service.CreateWhatsAppChannelAsync(tenantId, request, ct)));
    platform.MapPost("/tenants/{tenantId:guid}/whatsapp/channels/{channelId:guid}/assessment", async (Guid tenantId, Guid channelId, UpdateWhatsAppChannelAssessmentRequest request, IPlatformAdministrationService service, CancellationToken ct) => Results.Ok(await service.UpdateWhatsAppChannelAssessmentAsync(tenantId, channelId, request, ct)));
    platform.MapPost("/tenants/{tenantId:guid}/whatsapp/channels/{channelId:guid}/{action}", async (Guid tenantId, Guid channelId, string action, IPlatformAdministrationService service, CancellationToken ct) => { await service.SetWhatsAppChannelStatusAsync(tenantId, channelId, action, ct); return Results.NoContent(); });
    ...
    conversations.MapGet("/operators", async (IConversationAdministrationService service, CancellationToken ct) => Results.Ok(await service.GetAssignableUsersAsync(ct))).RequireAuthorization("ConversationOperators");
    ...
    conversations.MapPost("/{id:guid}/assign", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.AssignAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/release", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.ReleaseAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/transfer", async (Guid id, ConversationTransferRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.TransferAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/automation/pause", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.PauseAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/automation/resume", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.ResumeAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/close", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.CloseAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/reopen", async (Guid id, ConversationOperationRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.ReopenAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPatch("/{id:guid}/priority", async (Guid id, ConversationPriorityRequest request, IConversationAdministrationService service, CancellationToken ct) => { await service.SetPriorityAsync(id, request, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    conversations.MapPost("/{id:guid}/messages", async (Guid id, ManualConversationMessageRequest request, HttpRequest httpRequest, IConversationAdministrationService service, CancellationToken ct) => { await service.SendManualMessageAsync(id, request, httpRequest.Headers["Idempotency-Key"].ToString(), ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    app.MapGet("/api/conversation-queue", async ([AsParameters] HumanQueueListQuery query, IConversationAdministrationService service, CancellationToken ct) => Results.Ok(await service.GetHumanQueueAsync(query, ct))).RequireAuthorization("ConversationOperators");
    app.MapGet("/api/notifications", async (int page, int pageSize, IOperationalNotificationService service, CancellationToken ct) => Results.Ok(await service.ListAsync(page, pageSize, ct))).RequireAuthorization("ConversationOperators");
    app.MapGet("/api/notifications/summary", async (IOperationalNotificationService service, CancellationToken ct) => Results.Ok(await service.SummaryAsync(ct))).RequireAuthorization("ClinicStaff");
    app.MapPost("/api/notifications/{id:guid}/read", async (Guid id, IOperationalNotificationService service, CancellationToken ct) => { await service.MarkReadAsync(id, ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
    app.MapPost("/api/notifications/read-all", async (IOperationalNotificationService service, CancellationToken ct) => { await service.MarkAllReadAsync(ct); return Results.NoContent(); }).RequireAuthorization("ConversationOperators");
